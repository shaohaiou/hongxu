		...
		jQuery.extend( $.fn.jQchart,{
			...
			wrtGraph : function(op){	//200:y=700:100
				...
				switch(op.type){
					...
					case('improveline')  : it=this.drawImproveLine(op,it)  ;break;
			...
			/*
		   * 在大量数据存在的情况下可以挑选要显示的点的值，需要配置 config 中的 show值。
		   * author：RayZhao 2012-8-29 18：31 
		   * e-mail: zhao_lei@msn.com
		   * www.ingit.com 专业Gps车辆定位。
			 */
			drawImproveLine : function (op,it){
				$.each(op.data,function(index,value){
						op.rows=op.data[index];
						op.subclass = 'labelData';
						op.color		= 'orange'	;
						var strokeStyle=op.line_strokeStyle[index]||'#777';
						its.ctx.strokeStyle=strokeStyle;
						...
						var originalX = op.paddingL + op.xGapPaddingL;
						$.each(op.show,function(index,value){
								x = originalX + op.xGap * value;
								y = op.paddingT+ op.chartHeight - op.rows[value]*op.chartHeight/op.dataYmax ;
								if( x <= op.width && value<length){
