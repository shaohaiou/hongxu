...
namespace Hx.Components.Web
{
    public class HXContext
    {
        ...
        private static HXContext _context = null;
        ...
        private AdminInfo _admin = null;
        /// <summary>
        /// 后台管理员
        /// </summary>
        [JsonIgnore]
        public AdminInfo AdminUser
        {
            get
            {
                if (_admin == null)
                {
                    _admin = _httpContext.Session[GlobalKey.SESSION_ADMIN] as AdminInfo;
                    if (_admin == null && ManageCookies.GetCookie(GlobalKey.SESSION_ADMIN) != null)
                    {
                        int id = DataConvert.SafeInt(ManageCookies.GetCookie(GlobalKey.SESSION_ADMIN).Value);
                        if (id > 0)
                        {
                            AdminInfo admin = Admins.Instance.GetAdmin(id);
                            if (admin != null)
                            {
                                _httpContext.Session[GlobalKey.SESSION_ADMIN] = admin;
                                _admin = admin;
                            }
                        }
                    }
            ...
            {
                _admin = value;
                _httpContext.Session[GlobalKey.SESSION_ADMIN] = _admin;
            }
        }
        ...
        private HXContext(HttpContext context)
        {
            try
            {
        ...
        [JsonIgnore]
        public static HXContext Current
        {
            get
            {
                if (HttpContext.Current == null)
                {
                    if (_context == null)
                    {
                        _context = new HXContext();
                    }
                    return _context;
                }
                HttpContext httpContext = HttpContext.Current;
                HXContext context = null;
                context = httpContext.Items[GlobalKey.CONTEXT_KEY] as HXContext;
                if (context == null)
                {
                    context = new HXContext(httpContext);
        ...
        private static void SaveContextToStore(HXContext context)
        {
            if (context.Context != null)
            {
                context.Context.Items[GlobalKey.CONTEXT_KEY] = context;
            }
        }
        ...
        public bool AdminCheck
        {
            get
            {
                if (_httpContext.Session[GlobalKey.SESSION_ADMIN] != null)
                {
                    return true;
                }
                else if (ManageCookies.GetCookie(GlobalKey.SESSION_ADMIN) != null)
                {
                    int id = DataConvert.SafeInt(ManageCookies.GetCookie(GlobalKey.SESSION_ADMIN).Value);
                    if (id > 0)
                    {
